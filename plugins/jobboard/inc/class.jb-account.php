...
if (! class_exists('JobBoard_Account')) :
    ...
    class JobBoard_Account{
        ...
        function __construct()
        {
            ...
            add_action( 'jobboard_dashboard_not_logged', array($this, 'login_error'), 0);
            add_action( 'ndidate', array($this, 'login_error'), 0);
            add_action( 'jobboard_dashboard_employer', array($this, 'login_error'), 0);
            // hook user profile to show and edit specialisms
        ...
        function login_redirect($redirect_to, $requested_redirect_to, $user){
            if(!empty($user->errors)){
                wp_redirect(add_query_arg('failed', '1', jb_page_permalink('dashboard')));
            }
        ...
        function login_error(){
            ...
            switch ($_GET['failed']){
                case '1':
                    jb_notice_add( esc_html__( 'Email or password incorrect', JB_TEXT_DOMAIN ), 'error');
                    break;
            }
        }
        ...
        function additional_profile_fields($user){
            ...
            if(function_exists('jb_register')){
                ...
                $fields = $jb_register->get_custom_fields();
            }
            if(isset($fields['job_specialisms_group'])):
            $job_specialisms_group = get_user_meta($user->ID, 'job_specialisms_group', true);
            $verified = get_user_meta($user->ID, 'verified', true);
            $value = array();
            if(!empty($job_specialisms_group)){
                foreach ($job_specialisms_group as $user_meta_key){
                    $value[$user_meta_key] = get_user_meta($user->ID, $user_meta_key, true);
                }
            }
            $fields = array(
                'job_specialisms_group' => array(
                    'id'            => 'job_specialisms_group',
                    'type'          => 'group',
                    'title'         => esc_html__('Specialization Group', JB_TEXT_DOMAIN),
                    'subtitle'      => esc_html__('Specialization Group', JB_TEXT_DOMAIN ),
                    'value'         => $value,
                    'fields'        => array(
                        'job_specialisms' => array(
                            'id'            => 'job_specialisms',
                            'title'         => esc_html__('Specializations', JB_TEXT_DOMAIN ),
                            'subtitle'      => esc_html__('Select your specializations.', JB_TEXT_DOMAIN ),
                            'placeholder'   => esc_html__('Specializations',JB_TEXT_DOMAIN),
                            'type'          => 'select',
                            'options'       => jb_get_specialism_options(),
                        ),
                        'job_specialisms_license' => array(
                            'id' => 'job_specialisms_license',
                            'title' => esc_html__('License', JB_TEXT_DOMAIN),
                            'subtitle' => esc_html__('Upload your License file.', JB_TEXT_DOMAIN),
                            'type' => 'admin_media',
            ...
            ?>
            <h3><?php echo __('Specializations', JB_TEXT_DOMAIN); ?></h3>
            <script type='text/javascript'>
                jQuery(document).ready( function($) {
                    ...
                    mediaControl = {
                        ...
                        select: function() {
                            var attachment = mediaControl.frame().state().get('selection').first();
                            console.log(attachment);
                            self.parents(".file-media").find("input[type=hidden]").val(attachment.id);
                            self.parents(".file-media").find(".file-name").text(attachment.attributes.filename);
                        },
            ...
            <table class="form-table">
                <tr>
                    <th>
                        <label>
                            <?php esc_html_e('Specializations Listing', JB_TEXT_DOMAIN); ?>
                        </label>
                    </th>
                    <td>
                        <?php
                            jb_template_form_dynamic_field($fields);
                        ?>
                    </td>
                </tr>
            </table>
            <table class="form-table">
                <tr>
                    <th>
                        <label for="verified">
                            <?php esc_html_e('Verification', JB_TEXT_DOMAIN); ?>
                        </label>
                    </th>
                    <td>
                        <select name="verified" id="verified">
                            <option value="0" <?php echo esc_attr($verified?'':'selected'); ?>><?php esc_html_e('No', JB_TEXT_DOMAIN); ?></option>
                            <option value="1" <?php echo esc_attr($verified?'selected':''); ?>><?php esc_html_e('Yes', JB_TEXT_DOMAIN); ?></option>
                        </select>
                    </td>
                </tr>
            </table>
        ...
        function save_profile_fields($user_id){
            ...
            if (isset($_POST['job_specialisms_group'])) {
                $job_specialisms_group = array();
                foreach ($_POST['job_specialisms_group'] as $key => $value){
                    update_user_meta($user_id, $key, $value);
                    $job_specialisms_group[] = $key;
                }
                update_user_meta($user_id, 'job_specialisms_group', $job_specialisms_group);
            }

            if (isset($_POST['verified'])) {
                update_user_meta($user_id, 'verified', $_POST['verified']);
            }
        }
    }
