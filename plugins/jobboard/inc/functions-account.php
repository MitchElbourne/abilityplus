...
function jb_account_change_password( $user_id ) {
	...
	if ( strlen( $_POST['new-password'] ) <= 8 ) {
		...
		jb_notice_add( esc_html__( 'Your password must be greater than 8 characters.', JB_TEXT_DOMAIN ), 'error' );
	...
	if ( $_POST['new-password'] !== $_POST['confirm-password'] ) {
		...
		jb_notice_add( esc_html__( 'Your passwords do not match.', JB_TEXT_DOMAIN ), 'error' );
	...
	jb_notice_add( esc_html__( 'Successfully changed password.', JB_TEXT_DOMAIN ) );
}
...
function jb_account_custom_fields_value( $user_id = '', $fields ) {
	...
	foreach ( $fields as $k => $field ) {
        ...
        if ( $field['id'] == 'job_specialisms_group' ) {
            $job_specialisms_group = get_user_meta($user_id, 'job_specialisms_group', true);
            $value = array();
            if (!empty($job_specialisms_group)) {
                foreach ($job_specialisms_group as $user_meta_key) {
                    $value[$user_meta_key] = get_user_meta($user_id, $user_meta_key, true);
                }
            }
            $fields[ $k ]['value'] = $value;
        }
...
function jb_account_the_display_verified() {
    $verified = jb_account_get_display_verified();
    if($verified){
        echo '<span class="status status-featured">';
        esc_html_e('Verified');
        echo '</span>';
    }
}
...
function jb_account_get_display_verified( $user_id = '' ){
    global $jobboard_account;
    ...
    return $verified = get_user_meta($user_id, 'verified', true);
}
