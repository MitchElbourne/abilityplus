...
//    jb_get_template('users/location.php', array('location' => jb_account_get_location()));
    jb_get_template('users/location.php', array('location' => jb_account_the_country()));
}
...
function jb_template_group_dynamic_field($group_field_name, $field){
    $field = jb_parse_custom_fields($field);
    $class = array('field', 'col-xs-12', 'col-sm-12');
    $class[] = "col-md-" . $field['col'];
    $class[] = "field-" . $field['type'];
    ...
    $field['class'] = apply_filters("jobboard_group_field_{$field['type']}_class", $class);
    $field['class'] = implode(' ', $field['class']);
    $template_path = apply_filters("jobboard_group_field_{$field['type']}_template_part", '');
    $default_path = apply_filters("jobboard_group_field_{$field['type']}_default_path", '');
    $field = apply_filters("jobboard_group_field_{$field['type']}_args", $field);

    do_action('jobboard_before_group_field', $field);

    jb_get_template('fields/fields/' . $field['type'] . '.php', $field, $template_path, $default_path);

    do_action('jobboard_after_group_field', $field);
}
...
function jb_template_employer_resume_search()
{
    global $wp_query;
