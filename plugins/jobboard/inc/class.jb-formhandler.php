...
class JobBoard_FormHandler
{
    ...
    /**
     * Get the instane of JobBoard_FormHandler
     *
     * @return self
     */
        ...
        foreach ($fields as $field) {
            ...
            if($field['type'] == 'group'){
                if(isset($_POST[$field['id']])){
                    $group = array();
                    foreach ($_POST[$field['id']] as $group_field_id => $group_field){
                        $user_meta[$group_field_id] = $group_field;
                        $group[] = $group_field_id;
                    }
                    if(isset($_FILES[$field['id']])){
                        $group_files = $_FILES[$field['id']];
                        foreach ($group_files['name'] as $key => $value) {
                            $group[] = $key;
                            $old_group_files = get_user_meta($user_id, $key, true);
                            $old_group_files = !empty($old_group_files)?$old_group_files:array();
                            $convert_files = array();
                            foreach ($group_files['name'][$key] as $key_2 => $value_2){
                                $group_file = array(
                                    'name' => $group_files['name'][$key][$key_2],
                                    'type' => $group_files['type'][$key][$key_2],
                                    'tmp_name' => $group_files['tmp_name'][$key][$key_2],
                                    'error' => $group_files['error'][$key][$key_2],
                                    'size' => $group_files['size'][$key][$key_2]
                                );
                                $convert_files[$key_2] = $this->upload_files($group_file);
                                if(isset($old_group_files[$key_2])){
                                    if(!empty($convert_files[$key_2])){
                                        wp_delete_attachment($old_group_files[$key_2]);
                                    }
                                    else{
                                        $convert_files[$key_2] = $old_group_files[$key_2];
                                    }
                                }
                            }
                            $user_meta[$key] = $convert_files;
                        }
                    }
                    $user_meta[$field['id']] = $group;
                    continue;
                }
            }
